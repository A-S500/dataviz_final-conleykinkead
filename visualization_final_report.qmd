...
nailpolish_gel <- data.frame (name = c("Freshwater (gel)", "Pooh Bear", "Pineapples Have Peelings Too", "Anything Goes (gel)", "Addicted to Sparkle", "Ruby Red Glitz", "Jade Platinum", "Mellow Yellow (gel)", "604", "Iconic", "Princess Jasmine (gel)", "618", "614", "615", "DG11", "Hidden Treasure", "Green to Café Latte", "Crystal Ball", "622", "611", "613", "616", "617", "612", "608", "619", "609", "607", "621", "620", "610", "603", "606", "605"),
                          ...
                          color_hex = c("#A9CFDC", "#BE969D", "#DEBB75", "#B9A3B1", "#A8C6BF", "#733566", "#4291C8", "#EFE669", "#F7C9B7", "#CEE6E2", "#6BA8A2", "#EB5328", "#E1F14F", "#E93C82", "#A2C0B8", "#CE9AA4", "#8FA260", "#799DC9", "#CFB19A", "#E6CAD6", "#EB5B6D", "#EA526B", "#DF4126", "#98AFBC", "#FCEDBA", "#194584", "#DCB4E4", "#9793C9", "#A19AB7", "#603292", "#F1BAA2", "#EFDDE4", "#E0ABB1", "#AFE8E2"),
...
Need to color_family points to correspond with the color.

```{r}
ggplot(nailpolish, aes(color_family, year_acquired, fill = color_family)) + 
  geom_bin_2d()
```
...
```{r}
ggplot(nailpolish, aes(year_acquired, color_family, fill = color_family)) + 
  stat_bin_2d()
```

```{r}
ggplot(nailpolish, aes(year_acquired, finish, fill = color_family)) + 
  stat_bin_2d()
```

```{r}
ggplot(nailpolish, aes(year_acquired, brand, fill = color_family)) + 
  stat_bin_2d()
```
...
```{r}
ggplot(nailpolish, aes(year_acquired, color_family, color = color_family)) + 
  geom_jitter() +
  stat_ellipse()
```
